...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <h2 class="text-2xl font-bold mb-6">Editar Interventor</h2>

        <form action="{{ route('interventores.update', $interventor) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="id_persona" id="id_persona" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                    ...
                    @foreach($personas as $persona)
                        <option value="{{ $persona->id_persona }}" {{ $interventor->id_persona == $persona->id_persona ? 'selected' : '' }}>{{ $persona->nombre }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="id_especialidad" id="id_especialidad" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                    ...
                    @foreach($especialidades as $especialidad)
                        <option value="{{ $especialidad->id_especialidad }}" {{ $interventor->id_especialidad == $especialidad->id_especialidad ? 'selected' : '' }}>{{ $especialidad->nombre }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="nit" id="nit" value="{{ $interventor->nit }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
            </div>
