...
<div class="container mx-auto px-4 sm:px-8">
    <div class="py-8">
        <h2 class="text-2xl font-semibold leading-tight mb-5">Crear Nuevo Proyecto</h2>

        <form action="{{ route('proyectos.store') }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            ...
            <div class="mb-4">
                ...
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                    ...
                    @foreach($naturalezas as $naturaleza)
                        <option value="{{ $naturaleza->id_naturaleza }}" {{ old('id_naturaleza') == $naturaleza->id_naturaleza ? 'selected' : '' }}>
                            {{ $naturaleza->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                    ...
                    @foreach($rubros as $rubro)
                        <option value="{{ $rubro->codigo_rubro }}" {{ old('codigo_rubro') == $rubro->codigo_rubro ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                    ...
                    @foreach($sectores as $sector)
                        <option value="{{ $sector->id_sector }}" {{ old('id_sector') == $sector->id_sector ? 'selected' : '' }}>
                            {{ $sector->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                    ...
                    @foreach($lineasBase as $lineaBase)
                        <option value="{{ $lineaBase->id_linea_base }}" {{ old('id_linea_base') == $lineaBase->id_linea_base ? 'selected' : '' }}>
                            {{ $lineaBase->nombre }}
                        </option>
                    @endforeach
                </select>
